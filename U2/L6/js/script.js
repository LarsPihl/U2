...
// ------------------------------
var cityElem;//Global variabel för inmatad stad.
// Initiera globala variabler och koppla funktion till knapp
function init() {
	formElem = document.getElementById("booking");//Referens till sidans formulär.
	totalCostElem = document.getElementById("totalCost");//Referens till span-element för utskrift av totalpris.
	cityElem = document.getElementById("city");//Referens till fält inmatning av stad.
	let zipcodeElem = document.getElementById("zipcode");//Lokal variabel för inmatning av postnummer.
	let telephoneElem = document.getElementById("telephone");//Lokal variabel för inmatning av telefonnummer.
	let campaignCode = document.getElementById("campaigncode");//Lokal variabel för inmatning av kampanjkod.
	...
	//en knapp exempelvis byts mot en annan. En lokal variabel 'i' skapas för genomsökning.
	...
	//En lokal variabel 'i' skapas för genomsökning.
	...
	formElem.nrOfNights.addEventListener("change", calculateCost);//'calculateCost' anropas också när antal nätter
	//ändras, så att utskriften av den totala kostnaden direkt syns.

	checkIfFamilyRoom();//Valt rumsalternativ undersöks.
...
function calculateCost() {//Funktion som beräknar total kostnad av rumsbokningar.
	...
	for (let i = 0; i < formElem.roomType.length; i++) {//Radioknapparna för rumsval söks igenom, det rum som valts
		//motsvarar variabeln 'price' aktuella pris genom att radioknappens index motsvarar ett pris på samma index 
		//i arrayen "roomPrice". En lokal variabel 'i' skapas för genomsökning.
	...
	for (let i = 0; i < formElem.extra.length; i++) {//De valbara tilläggen genomsöks, om någon av dem är vald
